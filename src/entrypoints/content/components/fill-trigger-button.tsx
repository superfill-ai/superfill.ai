...
const STYLES = `
  * {
  ...
  .icon-btn {
    ...
    width: ${ICON_SIZE + ICON_PADDING * 2}px;
    height: ${ICON_SIZE + ICON_PADDING * 2}px;
...
export class FillTriggerButton {
  ...
  private onIconClickHandler: ((e: MouseEvent) => void) | null = null;
  private onFillClickHandler: ((e: MouseEvent) => void) | null = null;
  ...
  async mount(targetField: HTMLElement) {
    try {
      ...
      const buttonSize = ICON_SIZE + ICON_PADDING * 2;
      ...
      this.iconButton.setAttribute("aria-label", "Open superfill.ai menu");
      this.iconButton.setAttribute("title", "Fill with superfill.ai");
      this.iconButton.innerHTML = ICON_SVG;

      this.onIconClickHandler = (e: MouseEvent) => {
        e.preventDefault();
        e.stopPropagation();
        this.toggleDropdown();
      };
      this.iconButton.addEventListener("click", this.onIconClickHandler);
      ...
      this.fillButton.className = "fill-btn";
      this.fillButton.type = "button";
      this.fillButton.setAttribute("aria-label", "Fill with superfill.ai");
      this.fillButton.innerHTML = `
        ${ICON_SVG}
        <span>Fill with superfill.ai</span>
      `;

      this.onFillClickHandler = (e: MouseEvent) => {
        e.preventDefault();
        e.stopPropagation();
        void this.handleFillClick();
      };
      this.fillButton.addEventListener("click", this.onFillClickHandler);

      this.dropdown.appendChild(this.fillButton);
  ...
  remove() {
    try {
      if (this.onIconClickHandler && this.iconButton) {
        this.iconButton.removeEventListener("click", this.onIconClickHandler);
      }
      if (this.onFillClickHandler && this.fillButton) {
        this.fillButton.removeEventListener("click", this.onFillClickHandler);
      }
      if (this.onClickOutside) {
      ...
      this.dropdown = null;
      this.isDropdownOpen = false;
      this.onIconClickHandler = null;
      this.onFillClickHandler = null;
